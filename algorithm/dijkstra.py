...
class Graph(object):
    def __init__(self):
        self.map = defaultdict(dict) #store all data of graph; dict or defaultdict both same as default datatype for outer defaultdict
    ...
    def dijkstra(self, startVertex = 'A'):
        ...
        while(BMHM.notEmpty()):
            ...
            for adjVertex, weight in self.map[current].items():
                if BMHM.containsKey(adjVertex):
                    ...
                    if BMHM.weightOfKey(adjVertex) > distanceFromStartVertex:
                        ...
                        vertexToParent[adjVertex] = current

        print(vertexToParent)
        print(vertexToStartVertexDistance)
